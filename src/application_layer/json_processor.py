...
class JsonProcessor:
    def __init__(self, message_client: MessageClient,
        ...
        self.notification_sender_factory = NotificationSenderFactory(
            self.message_client)
    ...
    def _send_notification(self, index: int, log_to_notify) -> None:
        ...
        try:
            ...
            notification_sender = self.notification_sender_factory.get_sender(
                log_to_notify['type'])
            notification_sender.send(log_to_notify)
        except BadParametersError as e:
            self.processed_data_repository.append_invalid_item(
                index, log_to_notify)
        finally:
